...
class Color:
    ...
    def FormattedAsInteger(self, maxVal):
        ...
        return str(Remap01ToInt(self.r, maxVal)).rjust(rjustValue) + " " + str(Remap01ToInt(self.g, maxVal)).rjust(
            rjustValue) + " " + str(Remap01ToInt(self.b, maxVal)).rjust(rjustValue)
...
class LUT:
    ...
    def ColorFromColor(self, color):
        ...
        cubeSize = self.cubeSize
        return self.ColorAtInterpolatedLatticePoint(color.r * (cubeSize - 1), color.g * (cubeSize - 1),
                                                    color.b * (cubeSize - 1))
    ...
    def FromCubeFile(cubeFilePath):
        ...
        for line in cubeFileLines[cubeSizeLineIndex + 1:]:
            if len(line) > 0 and len(line.split()) == 3 and "#" not in line:
                ...
                # mDebug : checking errors:
                currentColor = Color(redValue, greenValue, blueValue)
                #print(redIndex, greenIndex, blueIndex)
                #print(type(redIndex), type(greenIndex), type(blueIndex))


                lattice[int(redIndex),int(greenIndex),int(blueIndex)] = currentColor
                currentCubeIndex += 1
